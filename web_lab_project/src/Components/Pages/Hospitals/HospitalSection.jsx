...
const HospitalSection = ({title, data, slider}) => {
    ...
    useEffect(() => {
        if(sliderText === 'slider-no-disponible'){
            const filteredData = data.filter((hospital => hospital.status === false))
            setHospitals(filteredData)
        }
        if(sliderText === 'slider-disponible'){
            const filteredData = data.filter((hospital => hospital.status === true))
            setHospitals(filteredData)
        }
